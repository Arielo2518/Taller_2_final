...
casen_2021_rm_actualizada <- casen_2021_rm %>%
  left_join(casen_comunas_2021, by = "folio") %>%  # Unión por 'folio'
  group_by(folio) %>%
  summarize(across(everything(), ~first(na.omit(.)))) %>%  # Elegir el primer valor no nulo
  filter(complete.cases(.))                        # Eliminar filas con valores faltantes

casen_2021_rm_actualizada

```{r}

casen_2021_rm_actualizada <- casen_comunas_2021 %>%
  left_join(casen_2021_rm, by = "folio") %>%  # Unión por 'folio'
  distinct() %>%                                  # Eliminar duplicados
  filter(complete.cases(.))   %>%
   arrange(folio)       # Filtrar filas sin NA                                    # Eliminar filas con NA
```



```{r}
casen_2021_rm_actualizada1 <- casen_comunas_2021 %>%
  left_join(casen_2021_rm, by = "folio") %>%  # Unión por 'folio'
  group_by(folio) %>%
  summarize(across(everything(), ~first(na.omit(.)))) %>%  # Elegir el primer valor no nulo
  filter(complete.cases(.))  %>%
  arrange(folio) # Eliminar filas con valores faltantes
...
casen_2021_rm_actualizada <- casen_2021_rm_actualizada %>%
  mutate_all(~ifelse(. %in% c(-88, -99, 97,98,99), NA, .))
```
